 ...
 */
function generateAccessibilityTree({ currentGlobalId, processIFrames = true, iframePath = '', includeHidden = false, nodeIdsToIgnore = [], }) {
    const roleTagMap = {
        role: {
            div: 'generic',
            span: 'text',
    ...
    function extractAttributes(node) {
        ...
        // extract the background image attribute separately
        const computedStyle = window.getComputedStyle(node);
        const backgroundImage = computedStyle.getPropertyValue('background-image');
        if (backgroundImage && backgroundImage !== 'none') {
            attributes['background-image'] = backgroundImage;
        }
        return attributes;
    }
    ...
    function getName(element) {
        ...
        const attributes = ['aria-label', 'placeholder', 'alt', 'title', 'value', 'name'];
    ...
    function isElementHidden(element, children) {
        ...
        //Check if the element is aria-hidden
    ...
    function createNode(node, currentIframePath) {
        ...
        const tag = node.nodeName.toLowerCase();
        const skippedTags = ['script', 'style', 'path', 'g', 'hr', 'br', 'noscript', 'head'];
        ...
        if (tag === 'html' && !iframe_path && !currentIframePath) {
        ...
        if (!includeHidden && isElementHidden(node, children)) {
            return null;
        }
        let node_dict = {
            role: role || 'generic',
