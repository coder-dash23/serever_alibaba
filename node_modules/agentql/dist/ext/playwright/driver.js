...
function _processFrame(frame, lastUsedId, iframePath, includeHidden) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        scriptContent += `generateAccessibilityTree({
    currentGlobalId: ${lastUsedId},
    processIFrames: false,
    iframePath: "${iframePath}",
    includeHidden: ${includeHidden}
  });`;
        const result = yield frame.evaluate(scriptContent);
...
function pwGetByPrompt(page_1, prompt_1) {
    return __awaiter(this, arguments, void 0, function* (page, prompt, timeout = driver_constants_1.DEFAULT_QUERY_ELEMENTS_TIMEOUT_SECONDS * 1000, includeHidden = driver_constants_1.DEFAULT_INCLUDE_HIDDEN_ELEMENTS, mode = 'standard') {
        const query = `{
    page_element(${prompt})
  }`;
        const response = yield pwQueryElements(page, query, timeout, includeHidden, mode);
        return response.page_element;
    });
}
function pwQueryElements(page_1, query_1) {
    return __awaiter(this, arguments, void 0, function* (page, query, timeout = driver_constants_1.DEFAULT_QUERY_ELEMENTS_TIMEOUT_SECONDS * 1000, includeHidden = driver_constants_1.DEFAULT_INCLUDE_HIDDEN_ELEMENTS, mode = 'standard') {
        const atree = yield pwGenerateAccessibilityTree(page, includeHidden);
        logger_1.logger.notice(QUERY_TAKE_TIME_MESSAGE(timeout / 1000));
        const response = yield (0, aql_server_service_1.aqlQueryElements)(query, atree, page.url(), mode, timeout);
        return response;
    });
}
function pwQueryData(page_1, query_1) {
    return __awaiter(this, arguments, void 0, function* (page, query, timeout = driver_constants_1.DEFAULT_QUERY_DATA_TIMEOUT_SECONDS * 1000, includeHidden = driver_constants_1.DEFAULT_INCLUDE_HIDDEN_DATA, mode = 'standard') {
        const atree = yield pwGenerateAccessibilityTree(page, includeHidden);
        logger_1.logger.notice(QUERY_TAKE_TIME_MESSAGE(timeout / 1000));
